/*演習3-10 List3-16(p.124)の関数binsearchxを利用するプログラムを作成せよ。
(二分探索を行いつつ、該当要素が複数存在した場合、最も先頭にある要素へのポインタを返却する)*/
...
/**
* 配列に対して指定値の2分探索を行い、値が存在した場合、該当要素の中で最も先頭にある要素へのポインタを返却
...
void* doBinarySearch(void* keyValue, void* baseArray, size_t arraySize, size_t objectSize, int(*comparisionFunction)(const void*, const void*))
{
	...
	if (arraySize > 0) {
		...
		for (; firstElement != lastElement;) {
			...
			//探索範囲中央の添字を計算
			centralElement = (firstElement + lastElement) / 2;

			//探索値と中央要素の持つ値の比較を行う
			comparisionValue = comparisionFunction(keyValue, reinterpret_cast<void*>(&arrayPointer[(centralElement * objectSize)]));
		...
		//探索値を持つ要素以前の要素に、探索値が存在するかどうかを調べるループ
		for (; centralElement > firstElement; --centralElement) {

			//前の要素と検討中の要素を比較
			comparisionValue = comparisionFunction(reinterpret_cast<void*>(&arrayPointer[(centralElement - 1) * objectSize]), reinterpret_cast<void*>(&arrayPointer[centralElement * objectSize]));
...
int main()
{
	...
	//探索範囲となる配列
	int baseArray[arraySize] = { 10, 0, 3, 5, 5, 5, 5, 100, -5, 1 };
