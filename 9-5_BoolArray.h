...
public:
	...
	*/
	...
	/**
	* BitVector型オブジェクトに添字演算子[]を適用できるようにする演算子関数
	* @param inputIndex 添字
	* @return
	* @author Sawa
	* @since 8.20
	*/
	BitVector operator[](int inputIndex)
	{
		...
		return BitVector(byteArrayPointer[inputIndex / CHARACTER_BITS], (inputIndex & (CHARACTER_BITS - 1)));
	}

	/**
	* BitVector型オブジェクトに添字演算子[]を適用できるようにする演算子関数　(const版)
	* @param inputIndex 添字
	* @return bool型の値
	* @author Sawa
	* @since 8.20
